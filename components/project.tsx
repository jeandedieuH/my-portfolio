          ...
          >
            <div className="lg:grid lg:grid-cols-2 lg:gap-2 sticky top-9">
              <div className="lg:pb-16">
                <div>
                  <div className="md:inline-flex  gap-2 items-center tracking-widest text-sm lg:text-md text-transparent bg-clip-text">
                    <span className="md:font-bold uppercase text-white">
                      {project.name}
                    </span>
                  </div>
                </div>
                <h3 className="text-xl mt-2 md:mt-5 text-primary md:text-3xl text-white font-bold">
                  {project.title}
                </h3>
                ...
                <ul className="flex flex-col gap-4 mt-4 md:mt-5">
                  {project.results.map((result) => (
                    <li
                      key={result.title}
                      className="font-sans flex gap-2 items-center text-xs md:text-base text-white/80"
                    >
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {project.tools.map((tool, index) => {
                    return (
                      <span
                        key={index}
                        className="bg-primary/20 text-white px-3 py-1 rounded-full text-xs border"
                      >
                        {tool}
                      </span>
                    );
                ...
                <div className="flex gap-2">
                  {project.link && (
                    <a href={project.link} target="_blank">
                      <Button
                        className="group mt-8 h-10 bg-white text-black hover:bg-white rounded-full"
                        asChild
                      >
                        <span className="flex gap-2 items-center text-sm mt-1">
                          Demo{" "}
                          <MoveRight className="h-4 w-4 -rotate-45 group-hover:rotate-0" />{" "}
                        </span>
                      </Button>
                    </a>
                  )}
              ...
              <div className="lg:relative">
                <img
                  className="mt-8 border-white/20 -z-30 border-l-2 border-t-2 rounded-t-3xl border-r-2 lg:border-r-0 p-2 -mb-4 md:-mb-0 lg:mt-0 lg:absolute lg:h-full lg:w-auto lg:max-w-none"
                  src={project.image}
                  alt={project.title}
                />
              </div>
